...
$(document).ready(function () {
    ...
    $('#choose-meron, #choose-wala, #choose-draw,#choose-ftd').click(function (e) {
        ...
        if (e.target.id == "choose-meron") {
            des += $('#meron-name').text() + " MERON @" + rate + '.<br/> Maxbet ' + playerLimit.maxbet_meron;
            betInfo.Bettype = 0;
        ...
        if (e.target.id == "choose-wala") {
            des += $('#wala-name').text() + " WALA @" + rate + '.<br/> Maxbet ' + playerLimit.maxbet_wala;
            betInfo.Bettype = 1;
            betInfo.CockId = $(this).attr('data-cock');
            if (playerLimit) {
        ...
        if (e.target.id == "choose-draw") {
            des += "DRAW @" + rate + '.<br/> Maxbet ' + playerLimit.maxbet_draw;
            betInfo.Bettype = 2;
            if (playerLimit) {
        ...
        if (e.target.id == "choose-ftd") {
            des += "FTD @" + rate + '.<br/> Maxbet ' + playerLimit.maxbet_draw;
            betInfo.Bettype = 3;
            if (playerLimit) {
                $('#input-stake').attr('placeholder', playerLimit.minbet_draw);
            }
        ...
        if (matchdata == null) {
            ...
            if (x.length > 0) {
                ...
                if (betInfo.Bettype == 1) {
                    ...
                    des += $('#wala-name').text() + " WALA @" + matchdata.matchinfo.wala_rate + '.<br/> Maxbet ' + playerLimit.maxbet_wala;

                } else if (betInfo.Bettype == 0) {
                    betInfo.OddRate = matchdata.matchinfo.meron_rate;
                    betInfo.CockId = matchdata.matchinfo.cid;
                    des += $('#meron-name').text() + " MERON @" + matchdata.matchinfo.meron_rate + '.<br/> Maxbet ' + playerLimit.maxbet_meron;

                } else if (betInfo.Bettype == 2) {
                    betInfo.OddRate = matchdata.matchinfo.draw_rate;
                    des += "BDD @" + matchdata.matchinfo.draw_rate + '.<br/> Maxbet ' + playerLimit.maxbet_draw;

                } else if (betInfo.Bettype == 3) {
                    betInfo.OddRate = matchdata.matchinfo.ftd_rate;
                    des += "FTD @" + matchdata.matchinfo.ftd_rate + '.<br/> Maxbet ' + playerLimit.maxbet_draw;
                }
                $('#betInfo').val(JSON.stringify(betInfo));
                $('#bet-description').html(des);
            }
