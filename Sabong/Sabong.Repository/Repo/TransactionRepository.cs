...
namespace Sabong.Repository.Repo
{
            ...
            try
            {
                using (var context = new s_dbEntities())
                {
                    ...
                    if (playerPt != null)
                    {
                        ...
                        returnTransaction.adminfbper = 100 -
                                                       (playerPt.firstparent + playerPt.secondparent + playerPt.thirdparent +
                                                        playerPt.fourthparent);
 ...
 ( SELECT bet_commission FROM   commission where agentid=0 and for_id={4} ) AS adminbetcomm", playerPt.player_id,
                            playerPt.firstparent_id,
        ...
        public List<transaction> GetAllTransaction(int userId, DateTime date)
        {
            ...
            {
                ex.LogError(string.Format("User: {0}, Date: {1}",userId,date));
                return null;
            }
        ...
        public List<transaction> GetAcceptedTransactions(int userId, int matchId)
        {
            ...
            {
                ex.LogError(string.Format("User: {0}, Match: {1}",userId,matchId));
                return null;
            }
        }
